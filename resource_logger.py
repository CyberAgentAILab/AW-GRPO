...
# Raw data collection functions
def get_memory_data():
    """Get raw memory usage data"""
...
def get_disk_data(path):
    """Get raw disk usage data for a specific path"""
...
def format_detailed(memory_data, disk_data_list, vram_data):
    """Format resource data in detailed multi-line format"""
    log_entry = "-" * 50 + "\n"
...
def format_summary(memory_data, disk_data_list, vram_data):
    """Format resource data in single-line summary format"""
...
def get_csv_header(disk_paths):
    """Generate CSV header based on monitored disk paths"""
...
def format_csv(memory_data, disk_data_list, vram_data, disk_paths):
    """Format resource data as CSV row"""
    timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    row = [
        timestamp,
    ...
    # Create a dictionary of disk data for easy lookup
...
def log_resources(disk_paths=None, interval=5, log_file="resource_log.txt", format_type="detailed", quiet=False):
    """Log system resources in the specified format
    
    Args:
        disk_paths: List of disk paths to monitor
        interval: Time interval between logging in seconds
        log_file: Path to log file
        format_type: Output format (detailed, summary, or csv)
        quiet: If True, suppress output to stdout (only write to log file)
    """
    ...
    with open(log_file, "a") as f:
        ...
        while True:
            ...
            # Initialize log_entry with a default value
            log_entry = ""
            ...
            if format_type == "detailed":
                ...
                # Default to detailed format if an invalid format is specified
                log_entry = format_detailed(memory_data, disk_data_list, vram_data)
            
            if not quiet:
                print(log_entry, end="")  # Print to stdout only if not in quiet mode
            
            f.write(log_entry)  # Always write to log file
            f.flush()
